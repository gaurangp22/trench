...
import {
    ...
    CheckCircle, XCircle, MessageSquare, Package
} from "lucide-react"
...
export function ManageOrders() {
    ...
    const loadOrders = async () => {
        setLoading(true)
        try {
            const data = await ServiceOrderAPI.getMyOrders('freelancer')
    ...
    const filteredOrders = orders
        .filter(o => {
            ...
            if (filter === 'active') return ['active', 'delivered', 'revision_requested'].includes(o.status)
    ...
    const getCounts = () => ({
        all: orders.length,
        active: orders.filter(o => ['active', 'delivered', 'revision_requested'].includes(o.status)).length,
    ...
    return (
        <DashboardLayout role="freelancer">
            <div className="space-y-6">
                ...
                >
                    <div>
                        <h1 className="text-3xl font-bold text-white mb-1">Manage Orders</h1>
                        <p className="text-zinc-400">Track and fulfill client orders</p>
                    </div>
                </motion.div>
                ...
                {loading ? (
                    ...
                    >
                        ...
                        <p className="text-sm text-zinc-500 mb-6 max-w-sm mx-auto">
                            {searchQuery
                                ...
                                : 'When clients order your services, they will appear here'}
                        </p>
                    </motion.div>
                ) : (
                    <div className="space-y-3">
                        {filteredOrders.map((order, i) => {
                            ...
                            const isActive = ['active', 'revision_requested'].includes(order.status)
                            const needsAction = order.status === 'revision_requested'
                                    ...
                                    <div className="p-5">
                                        <div className="flex flex-col lg:flex-row lg:items-center gap-4">
                                            ...
                                            <div className="flex items-center gap-2 pt-4 lg:pt-0 border-t lg:border-0 border-white/[0.06]">
                                                ...
                                                {isActive && (
                                                    <Link to={`/freelancer/orders/${order.id}`}>
                                                        <Button className="h-9 px-4 rounded-lg bg-purple-500/10 text-purple-400 hover:bg-purple-500/20 text-sm">
                                                            {needsAction ? 'View Revision' : 'Deliver'}
                                                        </Button>
                                                    </Link>
                                                )}
                                                <Link to={`/freelancer/orders/${order.id}`}>
